  ...
  list: [
      ...
      descriptions: [
        ...
        { content: '구현', weight: 'BOLD' },
        {
          ...
          descriptions: [
            {
              content:
                '카드 등록 및 결제 흐름(등록 시작 → 결제 → 결제 취소 → 등록 완료)을 템플릿 메서드 패턴으로 추상화. 공통 흐름은 상위 클래스에서 정의하고 PG사별 세부 로직만 하위 클래스에서 구현하여, 신규 PG사 추가 및 교체 시 하위 클래스만 새로 작성하면 대응되도록 설계',
            },
            {
              content:
                '운영 중 특정 PG사 장애 발생 시, DB에 저장된 활성 PG사 설정값 변경만으로 재배포 없이 신규 카드 등록 트래픽을 대체 PG사 구현체로 전환할 수 있도록 설계',
            },
          ],
        },
        {
          ...
          descriptions: [
            {
              content:
                '결제 실패를 비즈니스 실패(잔액 부족, 한도 초과 등)와 PG사 실패(응답 지연, 일시적 오류 등)로 구분하여 처리. 결제 프로세스를 대기, 결제 진행중, 결제 성공, 결제 실패 등 단계별 상태로 분리하고 각 단계의 처리 결과를 DB에 기록',
            },
            {
              content:
                '비즈니스 실패는 재시도 없이 고객에게 알림을 발송하고, PG사 실패 구간부터 재처리·재시도가 가능하도록 설계',
            },
          ],
        },
        {
        ...
        },
        {
          ...
          descriptions: [
            {
              content:
                'PG 처리 스케줄러 실행 결과를 매일 단계별로 모니터링하고, 처리 실패 및 이상 감지 시 Rocket.Chat REST API를 통해 즉시 알림이 발송되는 장애 알림 체계 구축. 실패 유형별로 알림 발송 기준을 세분화',
            },
          ],
        },
        ...
        {
          content:
            '기업 고객 30곳의 수기 정산 프로세스를 카드 자동결제로 전환하여 담당자의 수작업 및 누락 리스크 제거',
        },
        {
          content: 'PG사 장애 발생 시 재배포 없이 대체 PG사로 전환 가능한 운영 체계 확보',
        },
        {
          content:
            '이중화 환경에서 스케줄러 단일 실행을 보장하여 운영 기간 중 중복 결제 및 데이터 정합성 문제 미발생',
        },
        {
          content:
            '결제 장애 발생 시 전체 흐름 재실행 없이 실패 단계부터 복구 가능한 구조를 확보하고, 불필요한 재시도로 인한 중복 요청 및 리소스 낭비 방지',
        },
      ],
    },
    {
      ...
      descriptions: [
        ...
        {
          content:
            'JSP → Vue SPA 전환으로 서버 렌더링 의존도 제거, 클라이언트 사이드 렌더링 기반 구조 확보',
        },
        {
          content:
            '쿠키 기반 언어 처리 전환으로 도메인 분기에 따른 불필요한 리렌더링 제거, 페이지 전환 안정성 향상',
        },
        {
          content:
    ...
    },
    {
      ...
      descriptions: [
        ...
        {
          content:
            '엑셀 및 수기 방식으로 운영되던 지출경비 결재 프로세스에서 문서 누락 및 처리 지연 문제가 반복 발생하여 전자결재 시스템을 처음부터 설계·구축하여 결재 흐름 자동화 및 운영 효율화를 담당',
        },
        { content: '구현', weight: 'BOLD' },
        {
          ...
          descriptions: [
            {
              content:
                '팀원·팀장·총무 등 역할별 결재 권한을 Spring Security 기반 RBAC 구조로 분리 설계. 역할별 접근 가능한 결재 단계와 수행 가능한 액션(요청·승인·반려·취소)을 정의해 권한 오남용을 방지하고, 신규 역할이 추가되어도 코드 변경 없이 권한 설정만으로 대응 가능하도록 구성',
            },
          ],
        },
        {
          ...
          descriptions: [
            {
              content:
                '체력단련비, 연장근무일지 등 향후 추가될 결재 문서 유형을 고려해 공통 결재 프로세스를 추상화. 결재 상태(요청·승인·반려·취소)는 공통 흐름으로 정의하고, 문서 유형별 세부 로직만 별도 구현체에 담당시켜 신규 문서 유형이 생겨도 공통 흐름 코드는 그대로 재사용',
            },
          ],
        },
        {
        ...
        {
          content:
            '역할 기반 권한 분리로 결재 단계별 접근 제어 명확화, 권한 오남용 및 문서 누락 리스크 제거',
        },
        {
          content:
            '공통 결재 프로세스 추상화로 신규 결재 문서 유형이 추가돼도 공통 흐름은 그대로 두고 문서별 로직만 얹으면 되어, 반복적인 결재 시스템 개편 부담 감소',
        },
        {
          content:
            '단계별 알림 자동화로 수동 확인 없이 결재 진행 상황 실시간 파악, 처리 지연 문제 개선',
        },
      ],
    },
  ],
